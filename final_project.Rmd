...
We will start by randomly sampling out 30% of our data because of the memory limitations of R's randomForest library. We will then build a random forest using an 80/20 train/test data split on this subset. This split is pretty standard, but can be different if you have good reason to split differently.
...
Our error rate is 0.33%! Pretty fantastic!

###Cross-Validation

We will now perform 10-fold cross-validation to compare our large randomForest (default 500 trees), with a moderately sized random forest (100 trees). This process can be automated to test different fold values (e.g. 5 instead of 10), and test differently sized forests. One could then plot the different hyperparameter values against test accuracy to choose the best values (That would be how one should really tune hyperparameters. But for the purposes of this tutorial, this test will give us an idea of how performance may or may not be plateauing after reaching a certain hyperparameter value.)

```{r cross_valid}
results_df <- createFolds(subset$state, k = 10) %>%
